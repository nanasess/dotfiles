...
"
  (interactive)
  (let (apachectl-command)
    (setq apachectl-command
	  (list "sudo" apachectl-program-command args))
    (with-current-buffer (get-buffer-create apachectl-buffer-name)
      (erase-buffer)
      (buffer-disable-undo)
      (set-process-sentinel
       (apply 'start-process "apachectl-cmd" (current-buffer)
	      apachectl-command)
       (lambda (proc stat)
	 (cond ((zerop (process-exit-status proc))
		(message "apachectl...OK!"))
	       ((message "apachectl...NG!")
